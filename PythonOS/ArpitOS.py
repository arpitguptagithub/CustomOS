...
f1 = open("data/password.txt", "rb+")  # password file
f2 = open("data/background.txt", "r+")  # I have provided a background color check that you can change in the settings
background = f2.readline()
encdPass = f1.readline()
password = base64.b64decode(encdPass).decode("utf-8")
...
def tick():
    global AFTER    # uncomment this if you want to use the global AFTER variable
    global truth
    global time_present
    global ddate
    global day
    ...
    newday = new_time.split(":")
    clock.config(text=day + "      " + new_time + "  ")
    if newday[0] == "00" and newday[1] == "00":   # this is to update the day
        ddate = asctime().split()
        day = ddate[0] + " " + ddate[1] + " " + ddate[2]
    if new_time != time_present:
        time_present = new_time
        clock.config(text=day + "      " + new_time + "  ")
    if truth:
        try:
            AFTER = welcome_screen.after(200, tick)    # this is for the clock to update every 200 milliseconds
        except Exception:
...
def welcome(o=""):
    ...
    def confirm(event=None):  
        ...
        global clock
        p = e.get()
        if p == password:   # if the password is correct
            truth = False
            welcome_screen.after_cancel(AFTER)
            clock = None
            """Argument "y" tells that welcome screen exists and needs to be
            destroyed"""
            home_screen = Home("y")  # home screen
        elif p != password:
            tries -= 1
...
class Home:
    ...
    def tick1(self):
        ...
        if new_time != self.time_present:
            self.time_present = new_time
            self.clock.config(text=day + "      " + new_time + "  ")
        try:
